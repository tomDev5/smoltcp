...
impl<'a, T: 'a> RingBuffer<'a, T> {
    ...
    /// reutrn `Err(Empty)` if no such element was found.
    pub fn remove(&mut self, value: &T) -> Result<(), Empty>
    where
        T: Eq + Copy,
    {
        if self.is_empty() {
        ...
        for i in 0..self.length {
            if self.storage[self.get_idx_unchecked(self.read_at + i)] == *value {
                p = Some(i);
                break;
            }
        }
        ...
        if let Some(i) = p {
            for j in i..self.length - 1 {
                let new_index = self.get_idx_unchecked(self.read_at + j);
                let old_index = self.get_idx_unchecked(self.read_at + j + 1);
                self.storage[new_index] = self.storage[old_index];
            }
            self.length -= 1;
            Ok(())
        } else {
...
mod test {
    ...
    pub fn expand_empty() {
        ...
        assert_eq!(ring_buffer.enqueue_one(), Err(Full));
        ring_buffer.expand_storage();
        *ring_buffer.enqueue_one().unwrap() = 123;
        assert!(!ring_buffer.is_empty());
        assert!(ring_buffer.is_full());
        assert_eq!(*ring_buffer.dequeue_one().unwrap(), 123);
        assert!(ring_buffer.is_empty());
        assert!(!ring_buffer.is_full());
    }
    ...
    pub fn test_buffer() {
        ...
        assert!(ring_buffer.is_empty());
        assert!(!ring_buffer.is_full());
        assert_eq!(ring_buffer.dequeue_one(), Err(Empty));
        ring_buffer.enqueue_one().unwrap();
        assert!(!ring_buffer.is_empty());
        assert!(!ring_buffer.is_full());
        for i in 1..TEST_BUFFER_SIZE / 2 {
            *ring_buffer.enqueue_one().unwrap() = i;
            assert!(!ring_buffer.is_empty());
        }
        println!("ring buffer {:?}", ring_buffer);
        for i in 0..TEST_BUFFER_SIZE / 2 {
            assert_eq!(*ring_buffer.dequeue_one().unwrap(), i);
            assert!(!ring_buffer.is_full());
        }
        for i in 0..TEST_BUFFER_SIZE {
            *ring_buffer.enqueue_one().unwrap() = i;
        ...
        ring_buffer.expand_storage();
        assert!(!ring_buffer.is_full());
        *ring_buffer.enqueue_one().unwrap() = TEST_BUFFER_SIZE;
        assert!(ring_buffer.is_full());
        assert_eq!(ring_buffer.enqueue_one(), Err(Full));
        ...
        assert_eq!(ring_buffer.remove(&5), Err(Empty));
        assert_eq!(ring_buffer.remove(&4), Ok(()));
        assert_eq!(ring_buffer.remove(&4), Err(Empty));
